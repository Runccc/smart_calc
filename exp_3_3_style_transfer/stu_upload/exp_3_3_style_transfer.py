...
class VGG19(object):
    ...
    def load_image(self, image_dir, image_height, image_width):
        ...
        with Image.open(image_dir) as img:
            ...
            resized = img.resize((image_width, image_height), Image.BILINEAR)
        self.input_image = np.asarray(resized, dtype=np.float32)
    ...
    def save_image(self, input_image, image_shape, image_dir):
        ...
        output_image = output_image.resize((image_shape[1], image_shape[0]), Image.BILINEAR)
        os.makedirs(os.path.dirname(image_dir), exist_ok=True)
        output_image.save(image_dir)
